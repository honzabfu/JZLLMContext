...
- **Formátování výsledku** — globální přepínač v nastavení; výsledek lze zobrazit s Markdown formátováním nebo jako prostý text
...
- **Doplňkový kontext** — volitelné textové pole pod náhledem schránky; přidá se k vstupu před odesláním. Pokud systémový prompt obsahuje `{{kontext}}`, vloží se přímo do promptu místo připojení za vstup. Stiskem **Enter** se spustí výchozí akce. Při zadaném textu se zobrazí ikona × pro rychlé vymazání pole.
...
- **Zobrazovat výsledek s formátováním** — pokud je zapnuto (výchozí), výsledek se renderuje jako formátovaný Markdown (nadpisy, tučný text, kód, …); při vypnutí se zobrazí jako prostý text.
