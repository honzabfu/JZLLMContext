...
- [Záloha konfigurace](#záloha-konfigurace)
...
- **Test připojení** — tlačítko „Testovat připojení" v nastavení každého providera ověří, zda je API klíč a konfigurace funkční
...
- **Záloha konfigurace** — sekce „Záloha konfigurace" obsahuje dvě tlačítka:
...
- Base URL se ukládá v konfiguračním souboru, API klíč v Keychain

Po kliknutí na **Uložit** se u každého providera zobrazí ikona `✓` (zelená) nebo `✗` (červená) podle výsledku uložení do Keychain.

**Testování připojení**

Každý provider má tlačítko **Testovat připojení**, které odešle minimální testovací požadavek na API. Výsledek se zobrazí zelenou `✓` nebo červenou chybovou hláškou. Test vyžaduje uložený API klíč (a u Azure/Vlastní správně vyplněný endpoint).
